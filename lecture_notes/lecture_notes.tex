...
\begin{tikzpicture}[
    scale=0.1,
    node distance=1.5cm,
    layer/.style={rectangle, draw=blue!50, fill=blue!10, thick, minimum width=3cm, minimum height=1cm, align=center},
    arrow/.style={->, thick, >=stealth},
    label/.style={text width=4cm, align=center}
]

% Input
\node[layer] (input) {Input \\ (1, 2401)};

% Convolutional Layers
\node[layer, right=of input] (conv1) {Conv1D \\ $16$ filtros \\ kernel=5};
\node[layer, right=of conv1] (pool1) {MaxPooling \\ kernel=4};

\node[layer, right=of pool1] (conv2) {Conv1D \\ $32$ filtros \\ kernel=5};
\node[layer, right=of conv2] (pool2) {MaxPooling \\ kernel=4};

\node[layer, right=of pool2] (conv3) {Conv1D \\ $64$ filtros \\ kernel=5};
\node[layer, right=of conv3] (pool3) {MaxPooling \\ kernel=4};

% Flatten
\node[layer, right=of pool3] (flatten) {Flatten};

% Linear Layers
\node[layer, above right=1.5cm and 1cm of flatten] (linear1) {Dense \\ (oculto)};
\node[layer, right=of linear1] (linear2) {Dense \\ (oculto)};

% Output
\node[layer, right=of linear2] (output) {Embedding \\ $8$ dim};
...
\draw[arrow] (pool3) -- (flatten);
\draw[arrow] (flatten) |- (linear1);
\draw[arrow] (linear1) -- (linear2);
\draw[arrow] (linear2) -- (output);

% Labels
\node[below=0.2cm of input] (label_input) {\footnotesize Capa de entrada};
\node[below=0.2cm of pool1] (label_conv) {\footnotesize $3$ capas convolucionales};
\node[below=0.2cm of pool3] (label_flatten) {\footnotesize Aplanado};
\node[above=0.2cm of linear1] (label_linear) {\footnotesize $2$ capas densas};
\node[above=0.2cm of output] (label_output) {\footnotesize Embedding final};
