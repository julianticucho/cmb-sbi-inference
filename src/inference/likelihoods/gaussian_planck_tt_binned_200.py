...
class GaussianPlanckTTBinning200Likelihood(Likelihood):
    ...
    def initialize(self):
        self.cov_binned, self.lmin, self.lmax, _, _, _, _, _, _ = PlanckDataLoader.load_planck_data()
        self.cov_matrix = PlanckDataLoader.expand_cov_to_200_bins(
            self.cov_binned, 
            self.lmin, 
            self.lmax, 
            target_bins=200,
        ...
        obs_fn = ObservationFactory.create_planck_tt_200_observation()
        self.x_obs = obs_fn(theta_fid, seed=0).numpy() 
        self.binning_pipeline = PipelineFactory.create_planck_just_binning_200()

        self.log.info("GaussianPlanckTTBinning200Likelihood initialized")
