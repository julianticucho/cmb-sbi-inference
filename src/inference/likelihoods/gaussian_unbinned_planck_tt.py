...
from src.simulation.steps import RangeCutStep


class GaussianUnbinnedPlanckTTLikelihood(Likelihood):
    ...
    def initialize(self):
        ...
        self.cov_matrix = PlanckDataLoader.expand_cov_from_binned(self.cov_matrix, self.lmin, self.lmax).numpy()
        ...
        obs_fn = ObservationFactory.create_unbinned_planck_tt_observation()
        self.x_obs = obs_fn(theta_fid, seed=0).numpy() 
        self.cut_step = RangeCutStep(l_min=30, l_max=2477)

        self.log.info("GaussianUnbinnedPlanckTTLikelihood initialized")
    ...
    def logp(self, _derived=None, **params_values):
        ...
        x_theory_cut = self.cut_step(x_theory_full).numpy()
        diff = x_theory_cut - self.x_obs
